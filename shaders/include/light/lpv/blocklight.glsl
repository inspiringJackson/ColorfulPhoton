...
float lpv_distance_fade(vec3 scene_pos) {
	float distance_fade  = 0.5 * max_of(abs(scene_pos / vec3(voxel_volume_size)));
		  distance_fade  = linear_step(0.75, 5.0, distance_fade);
...
vec3 get_lpv_blocklight(vec3 scene_pos, vec3 normal, vec3 mc_blocklight, float ao) {
	...
	if (true) {//is_inside_voxel_volume(voxel_pos)
#ifndef NO_NORMAL
		...
		return mix(lpv_blocklight, mc_blocklight, 0.00001);
	} else {
